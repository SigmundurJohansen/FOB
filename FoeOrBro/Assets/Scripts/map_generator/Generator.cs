...
{
    public static Generator _instance;
    public static Generator Instance { get { return _instance; } }

    public GameObject texHeight;
    public GameObject texHeat;
    public GameObject texMoisture;
    public GameObject texBiome;
    ...
    public GameObject gmWidth;
    public GameObject gmHeight;



    [Header("Height Map")]
    public GameObject pTerrainOctaves;
    public GameObject pTerrainFrequency;
    public GameObject pForest;
    public GameObject pShallowWaters;
    public GameObject pSand;
    public GameObject pGrass;
    public GameObject pRock;
    public GameObject pDeepWater;
    ...
    [Header("Heat Map")]
    public GameObject pHeatOctaves;
    public GameObject pHeatFrequency;
    public GameObject pColdestValue;
    public GameObject pColderValue;
    public GameObject pColdValue;
    public GameObject pWarmValue;
    public GameObject pWarmerValue;
    ...
    [Header("Moisture Map")]
    public GameObject pMoistureOctaves;
    public GameObject pMoistureFrequency;
    public GameObject pDryerValue;
    public GameObject pDryValue;
    public GameObject pWetValue;
    public GameObject pWetterValue;
    public GameObject pWettestValue;
    ...
    [Header("Rivers")]
    public GameObject pRiverCount;
    public GameObject pMinRiverHeight;
    public GameObject pMaxRiverAttempts;
    public GameObject pMinRiverTurns;
    public GameObject pMinRiverLength;
    public GameObject pMaxRiverIntersections;
    ...
    protected virtual void Instantiate()
    {
        ...
        HeightMapRenderer = texHeight.GetComponent<MeshRenderer>();
        HeatMapRenderer = texHeat.GetComponent<MeshRenderer>();
        MoistureMapRenderer = texMoisture.GetComponent<MeshRenderer>();
        BiomeMapRenderer = texBiome.GetComponent<MeshRenderer>();
    ...
    {
        int _width = GetNumber(gmWidth, 512);
        int _height = GetNumber(gmHeight, 512);

        Width = _width;
        Height = _height;

        TerrainOctaves = GetNumber(pTerrainOctaves, 6);
        TerrainFrequency = GetNumber(pTerrainFrequency, 1.25);
        DeepWater = GetNumber(pDeepWater, 0.2f);
        ShallowWater = GetNumber(pShallowWaters, 0.4f);
        Sand = GetNumber(pSand, 0.5f);
        Grass = GetNumber(pGrass, 0.7f);
        Forest = GetNumber(pForest, 0.8f);
        Rock = GetNumber(pRock, 0.9f);

        HeatOctaves = GetNumber(pHeatOctaves, 4);
        HeatFrequency = GetNumber(pHeatFrequency, 3.0);
        ColdestValue = GetNumber(pColdestValue, 0.05f);
        ColderValue = GetNumber(pColderValue, 0.18f);
        ColdValue = GetNumber(pColdValue, 0.4f);
        WarmValue = GetNumber(pWarmValue, 0.6f);
        WarmerValue = GetNumber(pWarmerValue, 0.8f);

        MoistureOctaves = GetNumber(pMoistureOctaves, 4);
        MoistureFrequency = GetNumber(pMoistureFrequency, 3.0);
        DryerValue = GetNumber(pDryerValue, 0.27f);
        DryValue = GetNumber(pDryValue, 0.4f);
        WetValue = GetNumber(pWetValue, 0.6f);
        WetterValue = GetNumber(pWetterValue, 0.8f);
        WettestValue = GetNumber(pWettestValue, 0.9f);

        RiverCount = GetNumber(pRiverCount, 40);
        MinRiverHeight = GetNumber(pMinRiverHeight, 0.6f);
        MaxRiverAttempts = GetNumber(pMaxRiverAttempts, 1000);
        MinRiverTurns = GetNumber(pMinRiverTurns, 18);
        MinRiverLength = GetNumber(pMinRiverLength, 20);
        MaxRiverIntersections = GetNumber(pMaxRiverIntersections, 2);
    }
    ...
    public void SaveMap()
    {
        SaveSystem.SaveMap(Width, Height, Tiles);
    }
    ...
    public BiomeType GetBiomeType(Tile _tile)
    {
        return BiomeTable[(int)_tile.MoistureType, (int)_tile.HeatType];
    }
    ...
    private void AddMoisture(Tile _tile, float _amount)
    {
        MoistureData.Data[_tile.X, _tile.Y] += _amount;
        _tile.MoistureValue += _amount;
        if (_tile.MoistureValue > 1)
            _tile.MoistureValue = 1;

        //set moisture type
        if (_tile.MoistureValue < DryerValue) _tile.MoistureType = MoistureType.Dryest;
        else if (_tile.MoistureValue < DryValue) _tile.MoistureType = MoistureType.Dryer;
        else if (_tile.MoistureValue < WetValue) _tile.MoistureType = MoistureType.Dry;
        else if (_tile.MoistureValue < WetterValue) _tile.MoistureType = MoistureType.Wet;
        else if (_tile.MoistureValue < WettestValue) _tile.MoistureType = MoistureType.Wetter;
        else _tile.MoistureType = MoistureType.Wettest;
    }
    ...
    public float GetHeightValue(Tile _tile)
    {
        if (_tile == null)
            return int.MaxValue;
        else
            return _tile.HeightValue;
    }
    ...
    private void DigRiver(River _river, River _parent)
    {
        int intersectionID = 0;
        int intersectionSize = 0;

        // determine point of intersection
        for (int i = 0; i < _river.Tiles.Count; i++)
        {
            Tile t1 = _river.Tiles[i];
            for (int j = 0; j < _parent.Tiles.Count; j++)
            {
                Tile t2 = _parent.Tiles[j];
                if (t1 == t2)
                {
                    intersectionID = i;
        ...
        int counter = 0;
        int intersectionCount = _river.Tiles.Count - intersectionID;
        int size = UnityEngine.Random.Range(intersectionSize, 5);
        _river.Length = _river.Tiles.Count;

        // randomize size change
        int two = _river.Length / 2;
        int three = two / 2;
        int four = three / 2;
        int five = four / 2;
        ...
        if (count4 > _river.Length)
        {
            int extra = count4 - _river.Length;
            while (extra > 0)
            {
        ...
        // dig out the river
        for (int i = _river.Tiles.Count - 1; i >= 0; i--)
        {

            Tile t = _river.Tiles[i];
    ...
    // Dig river
    private void DigRiver(River _river)
    {
        int counter = 0;
    ...
    private void FindPathToWater(Tile _tile, Direction _direction, ref River _river)
    {
        if (_tile.Rivers.Contains(_river))
            return;

        // check if there is already a river on this tile
        if (_tile.Rivers.Count > 0)
            _river.Intersections++;

        _river.AddTile(_tile);

        // get neighbors
        Tile left = GetLeft(_tile);
        Tile right = GetRight(_tile);
        Tile top = GetTop(_tile);
        Tile bottom = GetBottom(_tile);
        ...
        if (left != null && left.GetRiverNeighborCount(_river) < 2 && !_river.Tiles.Contains(left))
            leftValue = left.HeightValue;
        if (right != null && right.GetRiverNeighborCount(_river) < 2 && !_river.Tiles.Contains(right))
            rightValue = right.HeightValue;
        if (top != null && top.GetRiverNeighborCount(_river) < 2 && !_river.Tiles.Contains(top))
            topValue = top.HeightValue;
        if (bottom != null && bottom.GetRiverNeighborCount(_river) < 2 && !_river.Tiles.Contains(bottom))
        ...
        if (min == leftValue)
        {
            ...
            {
                if (_river.CurrentDirection != Direction.Left)
                {
                    _river.TurnCount++;
                    _river.CurrentDirection = Direction.Left;
                }
                FindPathToWater(left, _direction, ref _river);
            }
        }
        else if (min == rightValue)
        {
            ...
            {
                if (_river.CurrentDirection != Direction.Right)
                {
                    _river.TurnCount++;
                    _river.CurrentDirection = Direction.Right;
                }
                FindPathToWater(right, _direction, ref _river);
            }
        }
        else if (min == bottomValue)
        {
            ...
            {
                if (_river.CurrentDirection != Direction.Bottom)
                {
                    _river.TurnCount++;
                    _river.CurrentDirection = Direction.Bottom;
                }
                FindPathToWater(bottom, _direction, ref _river);
            }
        }
        else if (min == topValue)
        {
            ...
            {
                if (_river.CurrentDirection != Direction.Top)
                {
                    _river.TurnCount++;
                    _river.CurrentDirection = Direction.Top;
                }
                FindPathToWater(top, _direction, ref _river);
            }
        }
    }
    ...
    private void FloodFill(Tile _tile, ref TileGroup _tiles, ref Stack<Tile> _stack)
    {
        // Validate
        if (_tile == null)
            return;
        if (_tile.FloodFilled)
            return;
        if (_tiles.Type == TileGroupType.Land && !_tile.Collidable)
            return;
        if (_tiles.Type == TileGroupType.Water && _tile.Collidable)
            return;

        // Add to TileGroup
        _tiles.Tiles.Add(_tile);
        _tile.FloodFilled = true;

        // floodfill into neighbors
        Tile t = GetTop(_tile);
        if (t != null && !t.FloodFilled && _tile.Collidable == t.Collidable)
            _stack.Push(t);
        t = GetBottom(_tile);
        if (t != null && !t.FloodFilled && _tile.Collidable == t.Collidable)
            _stack.Push(t);
    ...
    public float GetNumber(GameObject _inputfield, float _default)
    {
        string text = _inputfield.GetComponent<Text>().text;
        if (text != "")
    ...
    public int GetNumber(GameObject _inputfield, int _default)
    {
        string text = gmWidth.GetComponent<Text>().text;
        if (text != "")
    ...
    public double GetNumber(GameObject _inputfield, double _default)
    {
        string text = pTerrainFrequency.GetComponent<Text>().text;
        if (text != "")
            return Convert.ToInt32(text);
