...
public class ECSController : MonoBehaviour {
    ...
    private void CreateArchetypes(EntityManager _em)
    {
        ...
        var render = typeof(RenderMesh);
        var player = typeof(Player);
        var translate = typeof(Translation);
        var movement = typeof(MovementComponent);
        var scale = typeof(Scale);
        var renderBounds =  typeof(RenderBounds);
        var collider = typeof(Collider);
        var pathPos = typeof(PathPosition);
        var pathFollow = typeof(PathFollow);
        var rotation = typeof(Rotation);
        var selectable = typeof(Selected);
        var dest = typeof(DestinationComponent);
        var nonUniform = typeof(NonUniformScale);
        var human = typeof(Human);
        var kobolt = typeof(Kobolt);
